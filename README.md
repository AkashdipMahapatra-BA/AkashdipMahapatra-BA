...
| Repository (Short Name) | What This Repository Represents (HR-friendly) | Tech / Stack Involved | Repository Link |
|---|---|---|---|
| CI/CD RCA + Pipeline Architecture (Real Incident Case Study) | Production-style RCA showcasing how I diagnose and unblock CI/CD failures with full system context: event-driven AWS pipeline diagrams (S3 → EventBridge → Lambda container ETL), CI workflow breakdown, and a complete case study (IIDIP-12678) where pytest test-discovery + import-time execution issues blocked the pipeline; fixed via pytest fixtures, correct NaN handling, improved mocking, and tooling reliability. | AWS (S3, EventBridge), Lambda (containerized Python), CI/CD testing strategy, pytest fixtures, pandas/boto3, pre-commit automation, ruff/mypy/coverage via pipenv, AWS mocking patterns (moto). | https://github.com/AkashdipMahapatra-BA/Root-cause-analysis--CI-CD-fails |
| Datadog Works (Observability Runbooks) | Practical observability runbooks documenting real Datadog operational workflows (example: monitor priority governance via PR process instead of direct prod edits) with clear steps to reduce alerting mistakes and improve operational hygiene. | Datadog Monitors & Alerting, monitor governance/runbooks, PR-based operational changes (IaC mindset for monitoring). | https://github.com/AkashdipMahapatra-BA/Datadog-works |
| Vulnerability Fix Notes (Security Learning + Fix Patterns) | Security learning repo capturing common vulnerability patterns and remediation approaches (generalized, non-confidential), structured as: identify → assess → fix → verify.  | AppSec basics, secure configuration, dependency hygiene, vulnerability reasoning & remediation workflows.  | https://github.com/AkashdipMahapatra-BA/fix-vulnerabilities |
| Kafka Partition Learnings (Streaming Fundamentals) | Notes/experiments to build strong fundamentals on Kafka partitioning, throughput, ordering, and scaling tradeoffs—core concepts behind reliable event streaming systems.  | Apache Kafka concepts (topics/partitions/consumer groups), distributed systems fundamentals, performance/scaling thinking.  | https://github.com/AkashdipMahapatra-BA/Kafka-Partition-Reduce |
