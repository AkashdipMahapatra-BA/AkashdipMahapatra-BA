...
| Project / Repository | Description & Technologies | Link | Tags |
|---|---|---|---|
| CI/CD RCA + Pipeline Architecture (Real Incident Case Study) | Production-style RCA showcasing how I diagnose and unblock CI/CD failures with full system context: event-driven AWS pipeline diagrams (S3 → EventBridge → Lambda container ETL), CI workflow breakdown, and a complete case study (IIDIP-12678) where pytest test-discovery + import-time execution issues blocked deployments; fixed via pytest fixtures, correct NaN handling, improved mocking, and tooling reliability. | [View Repo](https://docs.datadoghq.com/monitors/) | `CI/CD`, `RCA`, `AWS`, `Lambda`, `S3`, EventBridge, `Python`, Pytest, Testing, `Quality Engineering` |
| **M1.0 Non‑PII Feed QA Sign‑Off (CC‑Schedules)**             | QA validated **Mercury 1.0 Cabin Crew Schedules Non‑PII feed** using an event-driven AWS pipeline (**S3 → EventBridge → Lambda → SNS FIFO → SQS FIFO**). Executed **15 NOT NULL negative validations**, confirmed **DLQ routing** and error causes via **CloudWatch Logs Insights** queries, and performed schema checks against the **Confluence Data Dictionary**. Built a repeatable QA harness by creating a dedicated FIFO SQS queue subscribed to the curated Non‑PII SNS topic; exported full-day CloudWatch logs as CSV and attached evidence screenshots for test sign‑off. | <ul><li>[Jira link](https://britishairways.atlassian.net/browse/IIDIP-21844)</li><li>[Confluence](https://britishairways.atlassian.net/wiki/spaces/IO/pages/104217839/Cabin+Crew+Schedules+-+Data+Dictionary)</li><li>[Repo link](https://github.com/AkashdipMahapatra-BA/Task-10---QA-validation-for-non-PII)</li></ul> | use <ul><li>aws s3</li><li>aws EventBridge</li><li>aws Lambda</li><li>aws CloudWatch</li><li>aws SNS (FIFO Topic)</li><li>aws SQS (FIFO Queue)</li></ul> `AWS`, `S3`, `EventBridge`, `Lambda`, `SNS FIFO`, `SQS FIFO`, `CloudWatch Logs Insights`, `DLQ`, `Data Quality`, `Non‑PII`, `QA Sign‑Off`, `ETL Validation` |
| Datadog Observability & RCA (Production Data Pipelines) | Practical runbooks and RCA documentation demonstrating deep-dive Datadog operational workflows for serverless AWS data pipelines. <br><br> **Key Investigations:** <br> <ul><li>**SNS Delivery Failures (IIDIP-22868):** Diagnosed a 27k+ anomaly spike in `aws.sns.number_of_notifications_failed` by correlating Datadog metrics with CloudWatch Logs Insights; proved the root cause was orphaned SNS `.fifo` subscriptions pinging deleted SQS test queues (`AWS.SimpleQueueService.NonExistentQueue`).</li><li>**S3 4xx Errors (IIDIP-22331):** Investigated `aws.s3.4xx_errors` and concurrent Lambda crashes; isolated the issue to `s3b-odin-prod-euwe1-glue-persistence-layer` and proved "Missing Upstream Data" (404 NoSuchKey) by verifying the absence of expected daily data partitions (`flightcrew.ground.changeset/`).</li></ul> | [View Repo](https://github.com/AkashdipMahapatra-BA/Datadog-works) | use <ul><li>Datadog Dashboards</li><li>Datadog Metrics Explorer</li><li>AWS SNS / SQS</li><li>AWS S3</li><li>AWS CloudWatch Logs Insights</li></ul> `Datadog`, Observability, Monitoring, Alerting, `RCA`, `AWS Serverless`, Data Pipelines, `Incident Response`, `Ops Excellence` |
| Vulnerability Fix (Security Learning + Fix Patterns) | Security learning repo capturing CVE remediation in containerized Lambda environments: multi-stage Docker builds, dependency upgrades (pip 25.3→26.0, gnupg2, python-libs), and post-deployment validation using CloudWatch metrics + ECR image analysis. Includes investigation methodology: CVE identification → Dockerfile restructuring → deployment verification → root cause analysis when metrics show "no data" (proving pre-existing pipeline dormancy vs. deployment issues). Real case: IIDIP-22185 fixing CVE-2025-68973, CVE-2025-12084, CVE-2026-1703 across 3 Lambda containers with 42% image size reduction (607MB→350MB). | [View Repo](https://github.com/AkashdipMahapatra-BA/fix-vulnerabilities) | use <ul><li>VsCode</li><li>GitHub Actions</li><li>aws ECR</li><li>aws lambda</li><li>aws CloudWatch</li></ul>  `CVE`, `Security`, `DevSecOps`, `Docker`, Multi-stage Builds, `AWS Lambda`, `ECR`, `CloudWatch`, Vulnerability Management, Container Hardening, `Root Cause Analysis`, Metrics `Investigation`,`Risk Thinking` |
| Kafka Partition Learnings (Streaming Fundamentals) | Notes/experiments to build strong fundamentals on Kafka partitioning, throughput, ordering, and scaling tradeoffs—core concepts behind reliable event streaming systems. | [View Repo](https://github.com/AkashdipMahapatra-BA/Kafka-Partition-Reduce) | use <ul><li>aws ECS</li><li>aws EC2</li><li>aws S3</li></ul> `Kafka`, Event Streaming, `Distributed Systems`, Performance, Scaling, `Data Engineering` |
| **Post-Deployment Validation Automation (Observability & CI/CD)** | Automated a manual post-deployment checklist (IIDIP-23046) for the **GSE Events Data Product** to reduce operational toil and ensure deployment health. <br><br> **Key Implementations:** <br> <ul><li>**Event-Driven Execution:** Architected a GitHub Actions workflow to dynamically trigger a Python script immediately upon deployment completion.</li><li>**AWS Observability Integration:** Utilized `boto3` to programmatically query **CloudWatch Metrics** (validating zero Lambda invocation errors) and **CloudWatch Logs Insights** (scanning DP Lambdas and API/SNS endpoints for exceptions).</li><li>**Datadog Alerting Check:** Integrated the `datadog-api-client` to automatically verify that zero Datadog monitors failed during the Change Request (CR) window.</li><li>**Automated Reporting:** Generated a dynamic, structured deployment checklist report (JSON/Excel format) with strict Yes/No health validations, standardizing the Dev environment sign-off process.</li></ul> | <ul><li>[Jira link](https://britishairways.atlassian.net/browse/IIDIP-23046)</li><li>[View Repo](https://github.com/AkashdipMahapatra-BA/Task-11---Automation)</li></ul> | use <ul><li>Python / Boto3</li><li>GitHub Actions</li><li>aws CloudWatch</li><li>aws Lambda</li><li>aws SNS</li><li>Datadog API</li></ul> `Automation`, `Python`, `Boto3`, `CI/CD`, `Observability`, `CloudWatch Logs Insights`, `Datadog`, `DevOps`, `Operational Excellence` |
