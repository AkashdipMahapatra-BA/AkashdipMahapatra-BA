...
|-------------------------|---------------------------------|-----------------------|-----------------|
| **CI/CD RCA Notes (ODIE Pipeline + Test Failures)** | A **deep-dive RCA + runbook repository** documenting CI/CD failures with **production-grade context**: full system architecture, Lambda ETL execution flow, CI/CD workflow dependencies, and post-fix verification steps. Includes a complete case study (IIDIP-12678) showing how a **pytest discovery failure** (tests defined dynamically/in a loop + module-level S3 actions) blocked the pipeline, and how it was fixed using **pytest fixtures**, proper NaN handling, and improved mocking + tooling discipline. | AWS (S3 event ingestion patterns; Lambda container runtime patterns), Python testing/tooling (**pytest fixtures**, pandas validation, boto3, pipenv), code quality automation (**pre-commit**, mypy/ruff/coverage execution via pipenv), AWS mocking for unit tests (**moto** patterns), CI/CD using Git-based PR checks + workflow gating and test collection discipline. </br></br> Python, CI/CD, DevOps, Git, `GitHub Actions`, YAML, Linux, `AWS` | <https://github.com/AkashdipMahapatra-BA/Root-cause-analysis--CI-CD-fails> |
| | |
...
| **Datadog Works**         | A **hands-on runbook / work-notes repo** documenting real operational steps for Datadog tasks. Example captured: **changing a Datadog monitor priority (P3 → P2)** for a Kafka/MSK metric monitor in **prod**, done via **“monitors-as-code”** (edit Terraform config → create branch → commit → push → PR), because direct priority edits in prod are not allowed in the process you follow. | Datadog **Monitors & Alerting** (monitor management and alerting concepts) [\[docs.datadoghq.com\]](https://docs.datadoghq.com/getting_started/monitors/), [\[docs.datadoghq.com\]](https://docs.datadoghq.com/api/latest/monitors/); **Terraform** Datadog provider / monitor-as-code approach [\[docs.datadoghq.com\]](https://docs.datadoghq.com/getting_started/integrations/terraform/), [\[registry.t...rraform.io\]](https://registry.terraform.io/providers/DataDog/datadog/latest/docs/resources/monitor); Git/GitHub PR workflow; Jira (work tracking); AWS Kafka/MSK metric monitoring (conceptual via monitor query). | <https://github.com/AkashdipMahapatra-BA/Datadog-works>                    |
